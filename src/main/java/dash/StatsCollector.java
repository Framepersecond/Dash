...
public class StatsCollector {
    ...
    public StatsCollector(JavaPlugin plugin) {
        this.plugin = plugin;
        this.dashVersion = plugin.getDescription() != null ? plugin.getDescription().getVersion() : "unknown";
    }
    ...
    private void collectSample() {
        ...
        int[] worldChunks = collectWorldChunksOnServerThread();
    ...
    private int[] collectWorldChunksOnServerThread() {
        if (!Bukkit.isPrimaryThread()) {
            try {
                Future<int[]> future = Bukkit.getScheduler().callSyncMethod(plugin, this::collectWorldChunks);
                return future.get(750, TimeUnit.MILLISECONDS);
            } catch (Exception ignored) {
                return new int[] { 0, 0, 0 };
            }
        }
        return collectWorldChunks();
    }

    private int[] collectWorldChunks() {
        ...
        return new int[] { overworldChunks, netherChunks, endChunks };
    }
