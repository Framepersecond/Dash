...
public class UpdatesPage {
    ...
    public static String render(String currentVersion, String latestVersion, boolean updateAvailable,
        ...
        String content = HtmlTemplate.statsHeader()
                ...
                + "    if (data && data.success) { showToast('Update downloaded! Dash will be updated on the next server restart.', 'success'); if(window.dashNavigate){dashNavigate(location.pathname+location.search,false);} }"
                ...
                + "    const data = await res.json();"
                + "    if (data && data.success) { showToast('Server restart scheduled.', 'success'); }"
