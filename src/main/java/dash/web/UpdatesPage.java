...
public class UpdatesPage {
    ...
    public static String render(String currentVersion, String latestVersion, boolean updateAvailable,
        ...
        if (updaterEnabled && updateAvailable && !updatePrepared) {
            primaryAction = "<button id=\"update-download-btn\" class=\"w-full sm:w-auto px-6 py-3 rounded-xl bg-primary/20 text-primary border border-primary/30 hover:bg-primary hover:text-black transition-all font-semibold\">Download &amp; Prepare</button>";
        }

        String restartAction = "";
        if (updatePrepared) {
            restartAction = "<button id=\"update-restart-btn\" class=\"w-full sm:w-auto px-7 py-3 rounded-xl bg-emerald-500 text-white hover:bg-emerald-400 transition-all font-bold shadow-lg\">Restart to Apply</button>";
        }

        String disabledHint = updaterEnabled
                ? ""
                : "<p class=\"text-sm text-rose-300\">Updater is disabled. Set <code class=\"text-rose-200\">updater.github-repo</code> and <code class=\"text-rose-200\">updater.github-token</code> in <code class=\"text-rose-200\">config.yml</code>.</p>";
        ...
        String content = HtmlTemplate.statsHeader()
                ...
                + "    if (data && data.success) { alert('Update downloaded! Dash will be updated on the next server restart.'); location.reload(); }"
                + "    else { alert((data && data.error) ? data.error : 'Update download failed.'); btn.disabled=false; }"
                + "  } catch (e) { alert('Update download failed.'); btn.disabled=false; }"
                + "});"
                + "document.getElementById('update-restart-btn')?.addEventListener('click', async function(){"
                + "  if (!confirm('Restart server now to apply update?')) return;"
                + "  const btn=this; btn.disabled=true;"
                + "  try {"
                + "    const res = await fetch('/api/update/restart',{method:'POST'});"
                + "    const data = await res.json();"
                + "    if (data && data.success) { alert('Server restart scheduled.'); }"
                + "    else { alert((data && data.error) ? data.error : 'Restart trigger failed.'); btn.disabled=false; }"
                + "  } catch (e) { alert('Restart trigger failed.'); btn.disabled=false; }"
                + "});"
                + "</script>";
