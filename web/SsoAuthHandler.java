...
public class SsoAuthHandler implements HttpHandler {
    ...
    private void persistNeoDashReturnUrls(Map<String, String> query) {
        ...
        if (isSafePanelUrl(masterUrl)) {
            plugin.getConfig().set("bridge.master_url", masterUrl);
            changed = true;
        }
        if (isSafePanelUrl(restartUrl)) {
            plugin.getConfig().set("bridge.restart_url", restartUrl);
            changed = true;
        }
        if (changed) {
            plugin.saveConfig();
    ...
    private void redirect(HttpExchange exchange, String location) throws IOException {
        exchange.getResponseHeaders().set("Location",
                BasePath.rewriteLocation(location, BasePath.forRequest(exchange, null)));
