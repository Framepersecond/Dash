...
public class PluginSettingsPage {
    ...
    public static String render(String message) {
        ...
        String serverIp = Dash.getInstance().getConfig().getString("server-ip", "");
        boolean sslEnabled = Dash.getInstance().getConfig().getBoolean("ssl-enabled", false);
        String panelUrl = Dash.getInstance().getConfig().getString("panel-url", "");
        String reportUrl = Dash.getInstance().getConfig().getString("report-url", "");
        ...
        String bridgeSecret = canWrite ? Dash.getInstance().getConfig().getString("bridge.secret", "") : "";
        ...
        String content = HtmlTemplate.statsHeader()
                ...
                + messageHtml
                + SettingsPage.featureControlPanel(canWrite)
                + "<div class='h-6'></div>"
                ...
                + "<input type=\"url\" name=\"report_url\" value=\"" + escapeHtml(reportUrl) + "\" placeholder=\"https://paper.example.com\" "
                + (canWrite ? "" : "readonly ")
                + "class=\"w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white font-mono placeholder-slate-500 focus:border-primary outline-none"
                + (canWrite ? "" : " opacity-50 cursor-not-allowed") + "\">\n"
                + "<p class=\"mt-1 text-xs text-slate-500\">Optional. Must point to this plugin dashboard, never NeoDash.</p>\n"
                + "</div>\n"
                ...
                // -- Language Card --
                + "<div class=\"rounded-3xl bg-glass-surface backdrop-blur-xl border border-glass-border overflow-hidden mb-6\" data-i18n-skip-form>\n"
                + "<div class=\"flex items-center gap-3 px-6 py-4 border-b border-white/5\">\n"
